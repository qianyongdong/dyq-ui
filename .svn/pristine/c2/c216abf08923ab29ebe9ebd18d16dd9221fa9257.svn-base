...
export const DList = defineComponent<ListProps>({
  ...
  setup(props, { slots, emit, expose, attrs }) {
    ...
    return () =>
      h('div', { ref: resizeRef, class: 'd-list' }, [
        h(
          'div',
          { class: 'd-list__content', style: style.value, ref: target },
          [
            list.value.map((item: any, index: number) =>
              h('div', { class: { move: !options.value.disabled } }, [
                slots.default?.({ item, index }) ||
                  h('div', { class: 'list__content__item' }, item),
              ])
            ),
          ]
        ),
