...
export const DViewerPage = defineComponent<ViewerPageProps>({
  ...
  setup(props, { slots, emit, expose }) {
    ...
    const style = computed(() => {
      const scale =
        props.autoSize && currentScale.value
          ? currentScale.value
    ...
    const render = debounce(async () => {
      console.log(props.transform.scale);
      ...
      if (
        ...
        (autoSize &&
          currentScale.value &&
          autoSizeScale.value === currentScale.value) ||
      ...
      try {
        ...
        if (autoSize) {
          autoSizeScale.value = renderScale;
          if (pageNum === 1) {
            events.onUpdateScale(renderScale);
          }
        }
    ...
    return () =>
      h(
        'div',
        {
          ...
          ref: pageRef,
          style:
            '--scale-factor:' + (currentScale.value || props.transform.scale),
        },
        [
          h(
            'div',
